...
const AFFIL = { 0: '無所属', 1: '流星学園', 2: '新星学園', 3: '所属3?', 4: 'ネビュラ?', 5: '流星附属', 7: '極星学園?', 8: '所属8?' };
...
function chipRow(el, items, key) {
  ...
  box.innerHTML = items.map(([v, label, color]) =>
    \`<span class="chip \${state[key] === v ? 'on' : ''}" data-v="\${v}" \${color ? \`style="color:\${state[key] === v ? color : ''}"\` : ''}>\${color ? \`<span class="dot" style="background:\${color}"></span>\` : ''}\${label}</span>\`
  ).join('');
